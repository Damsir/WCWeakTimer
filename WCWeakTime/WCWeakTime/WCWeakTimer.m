...
@property (assign, nonatomic) CFTimeInterval s, t, a;

@end

@implementation WCWeakTimer
...
+ (WCWeakTimer *)scheduledTimerWithTimeInterval:(NSTimeInterval)ti target:(id)aTarget selector:(SEL)aSelector userInfo:(id)userInfo repeats:(BOOL)yesOrNo {
    WCWeakTimer *timer = [[WCWeakTimer alloc] initWithTi:ti target:aTarget selector:aSelector userInfo:userInfo repeats:yesOrNo autorun:true isSafeBlock:false block:nil];
    [timer start];
...
+ (WCWeakTimer *)scheduledTimerWithTimeInterval:(NSTimeInterval)interval repeats:(BOOL)repeats block:(TimerAction)block {
    WCWeakTimer *timer = [[WCWeakTimer alloc] initWithTi:interval target:nil selector:NULL userInfo:nil repeats:repeats autorun:true isSafeBlock:false block:block];
    [timer start];
...
+ (WCWeakTimer *)timerWithTimeInterval:(NSTimeInterval)interval repeats:(BOOL)repeats block:(TimerAction)block {
    WCWeakTimer *timer = [[WCWeakTimer alloc] initWithTi:interval target:nil selector:NULL userInfo:nil repeats:repeats autorun:false isSafeBlock:false block:block];
...
+ (WCWeakTimer *)scheduledTimerWithTimeInterval:(NSTimeInterval)interval repeats:(BOOL)repeats target:(id)aTarget safeBlock:(TimerAction)block {
    WCWeakTimer *timer = [[WCWeakTimer alloc] initWithTi:interval target:aTarget selector:NULL userInfo:nil repeats:repeats autorun:true isSafeBlock:true block:block];
    [timer start];
    ...
    if (self.aTarget != nil && _isSafeBlock == false) {
        if (self.aSelector != NULL) {
            if ([self.aTarget respondsToSelector:self.aSelector]) {
                ...
                void (*func)(id obj, SEL sel, WCWeakTimer *t) = (void *)imp;
                func(self.aTarget, self.aSelector, self);
                
            } else {
                [self invalidate];
                NSLog(@"WCWeakTimer : unrecognized selector sent to instance!");
            }
        } else {
            [self invalidate];
            NSLog(@"WCWeakTimer : selector not found!");
        }
        ...
        if (_isSafeBlock) {
            if (self.aTarget == nil) {
                [self invalidate];
                self.action = nil;
                NSLog(@"WCWeakTimer : target not found!");
            } else {
        ...
        // 关闭 timer
        [self invalidate];
        NSLog(@"WCWeakTimer : selector and block not found!");
    }
}
